...
const OAuthButtons = ({ onGoogleClick, showEmailFallback = false }) => {
  ...
  // Simplified Google OAuth - automatically handles in-app browsers
  const handleGoogleLogin = () => {
    if (onGoogleClick) onGoogleClick();
    const googleAuthUrl = getApiUrl('auth/login/google');
    
    console.log('🚀 Starting Google OAuth with automatic browser detection');
    
    // Check if we're in an in-app browser
    if (shouldUseSystemBrowser()) {
      console.log('📱 In-app browser detected - opening in system browser');
      openInSystemBrowser(googleAuthUrl);
    } else {
      console.log('🖥️ Desktop browser detected - normal OAuth flow');
      window.location.href = googleAuthUrl;
    }
  };

  // Simple fallback - open in external browser
  ...
  return (
    <div className="space-y-4">
      {/* Main Google OAuth Button - automatically handles everything */}
      <button
      ...
      {process.env.NODE_ENV === 'development' && (
        <button
          ...
          className="w-full flex items-center justify-center px-4 py-2 border border-blue-300 rounded-md shadow-sm bg-blue-50 text-sm font-medium text-blue-700 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
